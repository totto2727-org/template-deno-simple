...
Use `nix develop` to obtain Deno from the pinned environment. CI loads the same environment with `eval "$(nix print-dev-env "$GITHUB_WORKSPACE#default")"` and runs `deno task ci`.
